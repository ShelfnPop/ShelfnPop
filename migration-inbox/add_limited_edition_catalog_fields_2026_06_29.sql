-- Add catalog-level limited edition fields and expose them through collection views.

alter table public.pop_catalog
  add column if not exists limited_edition boolean not null default false,
  add column if not exists limited_count integer,
  add column if not exists edition_notes text;

create or replace view public.user_collection_view with (security_invoker = true) as
select
  ...
  coalesce(uci.purchase_price, 0) as cost_each,
