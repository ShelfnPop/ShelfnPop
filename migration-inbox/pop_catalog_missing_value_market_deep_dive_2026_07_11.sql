-- Market-value deep dive for the 36-row missing-value batch.
...
with proposed(upc, estimated_value) as (
  values
    ('849803062262'::text,  7.00::numeric),
    ('889698147385'::text,  5.00::numeric),
    ('889698398824'::text,  7.00::numeric),
    ('889698601634'::text, 11.00::numeric),
    ('889698714150'::text, 11.00::numeric),
    ('889698762137'::text, 12.00::numeric),
    ('889698796972'::text, 17.00::numeric),
    ('889698821278'::text, 12.00::numeric),
    ('889698841146'::text, 21.00::numeric),
    ('889698844314'::text, 22.00::numeric),
    ('889698884006'::text, 35.00::numeric),
    ('889698904551'::text, 30.00::numeric)
)
...
-- Verification query (expected after first apply):
-- batch_rows = 36, valued_rows = 12, missing_rows = 24,
