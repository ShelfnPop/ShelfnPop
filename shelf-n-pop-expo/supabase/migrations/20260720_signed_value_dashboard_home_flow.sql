...
monthly_adds as (
  select
    uci.user_id,
    coalesce(sum(coalesce(uci.quantity, 1::bigint)), 0::numeric)::integer as pops_added_this_month,
    coalesce(sum(coalesce(uci.quantity, 1::bigint)::numeric * values.value_each), 0::numeric) as added_value_this_month
...
calc as (
  select
    base.*,
    case when total_pops = 0 then null::numeric else round(total_paid / total_pops::numeric, 2) end as average_paid_per_pop,
    case when total_pops = 0 then null::numeric else round(total_collection_value / total_pops::numeric, 2) end as average_value_per_pop,
    'Welcome back, ' || display_name as greeting_text,
    case
      when baseline_collection_value is null then added_value_this_month
      else total_collection_value - baseline_collection_value
    end as monthly_value_change,
    case
      when baseline_collection_value is null or baseline_collection_value = 0::numeric then null::numeric
      else round(((total_collection_value - baseline_collection_value) / baseline_collection_value) * 100::numeric, 1)
    end as monthly_value_change_percent,
    case
      when total_paid = 0::numeric then null::numeric
      else round((gain_loss / total_paid) * 100::numeric, 1)
...
select
  ...
  greeting_text,
  case
    when total_pops > 0 and gain_loss_percent is not null and gain_loss > 0::numeric then 'Your shelf is up ' || gain_loss_percent || '% overall.'
    when total_pops > 0 and gain_loss_percent is not null and gain_loss < 0::numeric then 'Your shelf is down ' || abs(gain_loss_percent) || '% overall.'
    ...
    when monthly_value_change > 0::numeric then 'Your collection value is up ' || monthly_value_change_percent || '% this month.'
    when monthly_value_change < 0::numeric then 'Your collection value is down ' || abs(monthly_value_change_percent) || '% this month.'
